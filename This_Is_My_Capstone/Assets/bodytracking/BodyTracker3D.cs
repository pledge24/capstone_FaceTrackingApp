...
public class BodyTracker3D : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        prefabs = new GameObject[91];
        offset = new Vector3(offsetX, offsetY, offsetZ);
    }
    private void Start()
    {
        hbm = GameObject.Find("AR Session Origin").GetComponent<ARHumanBodyManager>();
    }
    private void OnEnable()
    {
        hbm.humanBodiesChanged += BodyTrackedEvent;
    }
    private void OnDisable()
    {
        hbm.humanBodiesChanged -= BodyTrackedEvent;
    }
    void BodyTrackedEvent(ARHumanBodiesChangedEventArgs args)
    {
        foreach(ARHumanBody body in args.added)
        {
            NativeArray<XRHumanBodyJoint> joints = body.joints;

            rootPrefab.transform.position = body.transform.position + offset;
            rootPrefab.transform.rotation = body.transform.rotation;

            foreach(XRHumanBodyJoint joint in joints)
            {
                GameObject temp;
                if(!jointObjects.TryGetValue(joint.index, out temp))
                {
                    temp = prefabs[joint.index];
                    jointObjects.Add(joint.index, temp);
                    
                    Debug.Log(joint.index + "Added");
                }

                if (joint.tracked)
                {
                    //Update
                    temp.transform.localPosition = joint.anchorPose.position;
                    temp.transform.localRotation = joint.anchorPose.rotation;
                }
            }
        }
